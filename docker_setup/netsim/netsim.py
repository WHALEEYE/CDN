...
def is_click_running():
    return (
        "click"
        in check_both(
            "ps -e | grep click | grep -v grep", shouldPrint=False, check=False
        )[0][0]
    )


def servers_running():
    return is_apache_configured() or is_click_running()
    ...
    with open(click_conf, "w") as click_conf_file:
        if args.topology == "onelink":
            click_conf_file.write(
                ...
                % (
                    LINK_PORT[0],
                    APACHE_SERVER_PORT[0],
                    INITIAL_BANDWIDTH_BIT_PER_SEC,
                    CONTROL_PORT,
                )
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Launch bandwidth limited servers or multiple servers"
    )
    parser.add_argument(
        "topology",
        choices=["onelink", "twolink", "sharelink", "servers"],
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", default=False, help="only print errors"
    )
    parser.add_argument(
        "-v",
        "--verbose",
